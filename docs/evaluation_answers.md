# B5-1 평가 대비 설명 답안

## 1. 테이블을 왜 나눴는가?

엑셀처럼 한 표에 회원명, 이메일, 도서명, 카테고리명, 대여일을 모두 넣으면 같은 회원 정보와 도서 정보가 반복 저장된다. 그래서 `member`, `category`, `book`, `rental`로 나누었다. `rental`은 회원과 도서가 만나는 사건 테이블이다.

## 2. PK/FK와 1:N 관계

- PK는 각 행을 유일하게 식별하는 값이다. 예: `member.member_id`, `book.book_id`.
- FK는 다른 테이블의 PK를 참조하여 관계를 만든다.
- `member 1 : N rental`: 한 회원은 여러 번 대여할 수 있다.
- `book 1 : N rental`: 한 권의 책은 여러 번 대여될 수 있다.
- `category 1 : N book`: 한 카테고리는 여러 권의 책을 포함할 수 있다.

## 3. 컬럼 타입 선택 이유

SQLite 기준으로 문자열은 `TEXT`, 금액/수량/ID는 `INTEGER`, 날짜는 ISO 형식 문자열(`YYYY-MM-DD`)을 저장하기 위해 `TEXT`를 사용했다. SQLite는 별도 DATE 타입이 강하지 않으므로, 정렬 가능한 날짜 문자열을 사용했다.

## 4. INNER JOIN과 LEFT JOIN 차이

- INNER JOIN은 양쪽 테이블에 모두 매칭되는 행만 보여준다.
- LEFT JOIN은 왼쪽 테이블의 행을 모두 유지하고, 오른쪽에 매칭이 없으면 NULL로 보여준다.
- 이 프로젝트에서는 대여 기록이 없는 회원도 확인하기 위해 `member LEFT JOIN rental`을 사용했다.

관련 쿼리:
- Q05: 최근 대여 기록 + 회원명 + 도서명
- Q06: 도서 + 카테고리 연결
- Q07: 연체 기록 + 회원/도서/카테고리
- Q08: 대여 없는 회원 포함 대여 횟수

## 5. GROUP BY와 집계 함수

`GROUP BY`는 같은 기준의 행을 묶는다. `COUNT`는 개수, `SUM`은 합계, `AVG`는 평균을 계산한다. 예를 들어 회원별 대여 횟수는 `member_id`로 묶고 `COUNT(rental_id)`를 계산한다.

관련 쿼리:
- Q09: 회원별 대여 횟수/수수료 합계
- Q10: 카테고리별 도서 수/평균 가격
- Q11: 대여 상태별 건수/수수료 합계

## 6. 서브쿼리

서브쿼리는 쿼리 안에 또 다른 `SELECT`를 넣어 먼저 계산한 값을 바깥 쿼리의 조건으로 사용하는 방식이다. 이 프로젝트에서는 전체 평균 가격을 먼저 구한 뒤, 그 평균보다 비싼 도서를 조회했다.

관련 쿼리:
- Q12: 전체 평균 가격보다 비싼 도서

## 7. 인덱스 선택 이유

`rental(member_id, due_date)`에 인덱스를 만들었다. 이유는 특정 회원의 대여 기록을 반납기한 순으로 조회하는 요구가 자주 발생하기 때문이다. 이 인덱스는 `WHERE member_id = ? ORDER BY due_date` 형태의 조회에 유리하다.

관련 쿼리:
- Q15: `rental(member_id, due_date)` 인덱스와 실행 계획

## 8. 가장 복잡했던 쿼리 설명 예시

Q07은 `rental`, `member`, `book`, `category` 네 테이블을 연결한다. 먼저 대여 기록에서 `status = 'OVERDUE'`인 행만 고른다. 그 다음 회원 이름은 `member`, 도서명은 `book`, 카테고리명은 `category`에서 가져온다. 이 쿼리로 연체 기록을 사람이 읽기 쉬운 형태로 확인할 수 있다.

## 9. Q13/Q14 데이터 변경 안전성

Q13 `UPDATE`와 Q14 `DELETE`는 데이터를 직접 변경하는 쿼리이다. 그래서 실행 전 대상 행을 `SELECT`로 확인하고, `WHERE rental_id = ...`처럼 PK 기준의 명확한 조건을 사용했다. 자동 실행 스크립트에서는 원본 DB가 아니라 임시 DB 복사본에서 실행되므로 제출용 원본 데이터는 훼손되지 않는다.

관련 쿼리:
- Q13: 대여 상태를 OVERDUE로 변경
- Q14: 테스트 대여 기록 삭제

## 10. 보너스 미니 리포트 설명

이 DB로 확인할 수 있는 핵심 지표는 월별 대여 건수, 인기 도서 TOP 5, 연체율이 높은 회원이다. 월별 대여 건수는 운영 추이를 보고, 인기 도서는 추가 구매나 추천에 활용하며, 연체율이 높은 회원은 반납 알림과 대여 정책 관리에 활용할 수 있다. 실제 SQL과 결과는 `docs/bonus.sql`, `results/bonus_results.txt`에 정리했다.

## 11. FK 오류 데모 증빙

`docs/fk_error_demo.sql`은 존재하지 않는 `member_id = 999`를 참조하는 대여 기록을 일부러 입력한다. SQLite에서 `PRAGMA foreign_keys = ON` 상태로 실행하면 `FOREIGN KEY constraint failed`가 발생하고, 이 실패 로그는 `results/fk_error_demo.txt`에 저장했다. 따라서 FK가 선언만 된 것이 아니라 실제로 잘못된 참조를 막는다는 점을 보여줄 수 있다.
